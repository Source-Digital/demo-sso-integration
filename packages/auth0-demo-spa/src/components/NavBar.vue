...
export default {
  ...
  setup() {
    ...
    return {
      ...
      logout() {
        // send logout message to iframe via postMessage
        (document.getElementById('sourcesync-frame') as HTMLIFrameElement | null)?.contentWindow?.postMessage?.({ event: 'logout', context: 'sourcesync' }, 'http://localhost:3100');
